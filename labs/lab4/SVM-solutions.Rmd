...
```{r}
banknotes <- read.csv("https://archive.ics.uci.edu/ml/machine-learning-databases/00267/data_banknote_authentication.txt",
                      header = FALSE,
                      col.names = c("variance",
                                    "skew",
                                    "curtosis",
                                    "entropy",
                                    "class"))
...
```{r}
intrain <- sample(1:nrow(banknotes), nrow(banknotes)/2)

train <- banknotes[intrain, ]
test <- banknotes[-intrain, ]
```
...
ggpairs(train, mapping = aes(color = class),
        lower = list(continuous = wrap(ggally_points,
                                       alpha = .25,
                                       shape = 1)),
        diag = list(continuous = wrap(ggally_densityDiag,
                                      alpha = .25)))
```
...
```{r}
plot(m, data = test, formula = entropy ~ skew)
```

  * Generate a confusion matrix using the test data as the criterion.
  
```{r}
table(test$class, predict(m, test))
```
...
m <- tune(svm,
          class ~ .,
          data = train,
          ranges = list(gamma = 2^(-15:1), cost = 2^(-8:-2)))
m
```

```{r}
ggplot(m$performances, 
       mapping = aes(x = gamma, y = error)) + 
  geom_line() + 
  facet_wrap(~cost, labeller = label_both)
```
...
```{r}
table(test$class, predict(m$best.model, test))
```
