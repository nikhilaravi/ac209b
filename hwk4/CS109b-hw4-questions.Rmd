...
With this formula we get an intercept and slope for every district, the only difference is if we kept overall model intercept int the model than for district 101 we would not get a separate term (it would be the overall intercept).

When using factor covariates, there is usually a level (in this case district) that is taken to be baseline. Therefore the coefficient for district 101 is just the baseline coefficient. 

There is an overall slope for living.children which needs to be added to the slope for each of the districts (treating the first one as district 101). The district specific coefficient do not *replace* the overall slope, but instead *add* to it.
...
plot(district_labels, children_district_coeffs_unpooled_added_baseline)
points(pooled_coeffs)
ggplot(model_coeffs_compare, aes(x=district, y=all_coeffs, color=labels)) + geom_point() + ylim(c(-2,2))
use_counts = aggregate(contraceptive_use ~ district, bangladesh_data, sum)
ggplot(use_counts, aes(x=district, y=contraceptive_use)) + geom_point()
```
```{r}
...
cat('districts with top 10 counts', districts_with_top_counts)

# districts with largest unpooled coefficients 
top_coeff_indices = sort(children_district_coeffs_unpooled_added_baseline, decreasing=TRUE, index.return=TRUE)
cat('\ndistricts with largest unpooled coefficients ', district_labels[top_coeff_indices$ix[0:10]])

# districts with largest hierarchichal coefficients 
top_coeff_indices = sort(mcmc_coeff_districts_mean, decreasing=TRUE, index.return=TRUE)
cat('\ndistricts with largest hierarchichal coefficients ', district_labels[top_coeff_indices$ix[0:10]])
```
