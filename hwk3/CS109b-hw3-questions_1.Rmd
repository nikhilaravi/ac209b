...
```{r}
#----Naive Bayes Model--------#
test_predictions = predict(naiveBayes(V1 ~ ., data = author_train), newdata = author_test)
#test_predictions = ifelse(test_predictions== 'AaronPressman', 1, 0)

# confusion matrix
confmat(author_test$V1, test_predictions)

# overall accuracy
cat('\noverall accuracy ',  length(which(author_test$V1==test_predictions))/length(test_predictions)*100, '%')
```
...
# yA = as.numeric(apply(train[train$V1 == 'Aaron', -1], 2, sum))
# label.cv = testset[,1]
# label <- ifelse(labels.cv == 'aaron', 1, 0)
# 
...
pA_train <- apply(author_train[-1], 1, function(x) {
...
confmat(author_train$V1,
        factor(pA_train > .5,
               labels = c("AaronPressman", "AlanCrosby")))

pA_test <- class_probability(author_test[-1], author_test[1], 1)

confmat(author_test$V1,
        factor(pA_test < .5,
               labels = c("AaronPressman", "AlanCrosby")))
...
crossval = function(train, param_val, k) {
  ...
  for(i in 1:num_param){
    ...
    for(j in 1:k){
      ...
      actual_train <- sapply(train$V1, as.numeric)# asnumeric changes factors to 1, 2, 3 etc but we want 1 or 0
      actual_train <- replace(actual_train, actual_train==2, 0) # changes factors from 2 to 0
      pA_train = class_probability_train_test(train[-1], train[1], alpha=param_val[i])
...
pA_train = class_probability(author_test[-1], author_test[1], alpha=best_alpha)
predicted_vals = factor(pA_train < 0.5, labels=c("AaronPressman", "AlanCrosby"))
confmat(author_test$V1, predicted_vals)
cat('\noverall accuracy: ',  accuracy(author_test$V1, predicted_vals), '%')
...
alpha =1 implies a uniform prior for the two author groups. When alpha is small the prior distribution of each observation will only be made up of a few groups. As alpha grows larger, the prior suggests that the observation's distribution will be made up of more classes, and as alpha grows very large, the prior suggests that each observation will be represented almost equally by each group. 

- Do the above optimization. What is the optimal value of $\alpha$? What is your final log-loss prediction error?

The optimal value of alpha is 0.1 resulting in a log loss of 0.000129. 

Cross Validation results in a linear plot of log loss vs alpga. There are 100 training datapoints and 100 variables (and hence 99 parameters in the Dirichlet-Multinomial model), therefore with 5-fold cross validation we would have only 20 testing data points per fit and the model is bound to overfit resulting in a very low log loss. 
...
class_probability_train_test = function(train, test, class, alpha, nsims) {
...
for(n in nsims) {
  ...
  pa1 <- class_probability_train_test(author_train[-1],
                         author_test[-1],
                         author_train[[1]],
...
# ##### Comparing 0-1 loss off arbitrary alpha
# ## confusion matrix, as proportions.
# confmat(train_1$V1,
#       factor(class_probability(train_1[-1], test_1[1], 1)  < .5,
#               labels = c("AaronPressman", "AlanCrosby")))
...
Increasing the number of simulations does not have a noticeable effect on the prediction accuracy. Given the the nature and size of the dataset (100 observations and 100 features) and the magnitude of the oscillations in the log loss and 0-1 loss, the number of simulations will only cause the log loss to decrease. 
...
$E[R_k]$ can be interpreted as signature words of each author - words which may be representative of their style of writing or the topics they write about. 

- Do you find visible differences in the authors' choice of vocabulary? 

Comparing the top 25 words for author A and author B, Author A's words are related to politis, featuring words such as 'clinton', 'unions', 'policies'.  For Author B, the words appear to be related to europe, with words in portugese, and names of european cities such as warsaw and zagreb.  
...
```{r}
#------- SINGLE LOGISTIC REGRESSION MODEL FOR THE ENTIRE TRAINING SET-----
data_2_train
fit0 <- glm(contraceptive_use ~ ., data = data_2_train,
...
total_test_rows = nrow(data_2_test)
...
for (d in districts){
  ...
  average_accuracy = average_accuracy + (nrow(subset_districts_test)/total_test_rows)*district_accuracy
...
```{r}
cat('average accuracy from a district specific model: ', average_accuracy, '%' )
```

Due to the large variation in contraceptive usage between the different regions, the average weighted predictive accuracy of a district specific model is much higher than a glm fit to the entire data set. 
...
```
```{r}
fit_stan_glm <- stan_glm(contraceptive_use ~ ., data = data_2_train, 
                 family = binomial(link = "logit"))

## we need to do a little work to get predictions.
pd1 <- posterior_predict(fit_stan_glm,
                        newdata = data_2_test,
                        draws = 2000)

pd1 <- apply(pd1, 2, mean)

confmat(data_2_test$contraceptive_use,
        factor(pd1 > .5,
               labels = c(1, 0)))
```

```{r}
accuracy(data_2_test$contraceptive_use,
        factor(pd1 > .5,
               labels = c(1, 0)))
```
